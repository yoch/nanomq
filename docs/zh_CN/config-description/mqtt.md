...
mqtt = {
  ...
  log_dropped_messages = true # 为每个受影响的连接记录第一条丢弃警告。
...
| `log_dropped_messages` | 为每个受影响的连接记录第一条消息丢弃警告。禁用警告输出后，丢弃计数器仍然可用。 | `true`、`false`（默认值：`true`） |
...
更改 `log_dropped_messages` 后需要重启 broker。

::: tip

除 `log_dropped_messages` 外，以上 MQTT 相关配置均支持热升级，保存配置文件即生效。
