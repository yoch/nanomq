...
| nanomq_messages_received      | counter        | Broker 接受的 PUBLISH 消息数量   |
| nanomq_messages_sent          | counter        | Transport 完成的 PUBLISH 投递数量 |
| nanomq_messages_dropped       | counter        | 因无路由或 transport 目标不可用而拒绝的 PUBLISH 消息数量，包括出站队列满和 pipe 已断开 |
...
当一条消息被路由到多个订阅者时，每个成功完成的订阅者投递都会增加一次
`nanomq_messages_sent`。仅完成路由不会增加该计数器；必须等待 transport
报告发送成功。
