...
mqtt = {
  ...
  log_dropped_messages = true # Log the first drop warning for each affected connection.
...
| `log_dropped_messages` | Logs the first dropped-message warning for each affected connection. Drop counters remain available when warning output is disabled. | `true`, `false` (default: `true`) |
...
Changing `log_dropped_messages` requires a broker restart.

::: tip

Except for `log_dropped_messages`, these MQTT configurations support hot
upgrading and take effect after the configuration file is saved.
