...
| nanomq_messages_received      | counter        | PUBLISH messages accepted by the broker |
| nanomq_messages_sent          | counter        | PUBLISH deliveries completed by the transport |
| nanomq_messages_dropped       | counter        | PUBLISH messages rejected because no route or transport destination was available, including full egress queues and disconnected pipes |
...
A message routed to multiple subscribers contributes one completed delivery per
subscriber to `nanomq_messages_sent`. Routing a message does not increment this
counter until the transport reports a successful send.
